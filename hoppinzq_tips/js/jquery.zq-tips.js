(function($) {
	...
	var time = 750;
	$.fn.tips = function(options) { //用以绑定该组件到dom元素上，已弃用
		let defaults = {
			$element: $(this),
			defaultTipAnim: {
				time: time + 'ms',
	...
	$.tips = function(tipsOption) {
		...
		let obj = {
			initCss: function() {
				...
				let thisCss = [];
				thisCss.push(
					'div#zq-tips-page-overlay{opacity:0.75;position:fixed;inset:0px;background:#000;top:0;left:0;bottom:0;right:0;display:none;width:100%;height:100%;zoom:1;filter:alpha(opacity=75);z-index:100002 !important;transition:all .3s}'
					);
				thisCss.push(
					'div#zq-tips-highlighted-element-stage{position:absolute;top:0;left:0;height:50px;width:300px;background:#fff;z-index:100003 !important;display:none;border-radius:2px;transition:all .3s}'
					);
				thisCss.push(
					'.zq-tips-highlighted-element-stage{position:absolute;top:0;left:0;background:#fff;z-index:100003 !important;display:none;border-radius:2px;transition:all .3s}'
					);
				thisCss.push(
					'div#zq-tips-item{display:none;left:440px;top:38px;position:absolute;background:#fff;color:#2d2d2d;margin:0;padding:15px;border-radius:5px;min-width:250px;max-width:300px;box-shadow:0 1px 10px rgb(0 0 0 / 40%);z-index:1000000000}'
					);
				thisCss.push(
					'.zq-tips-fix-stacking{z-index:auto;opacity:1 !important;transform:none !important;-webkit-filter:none !important;filter:none !important;perspective:none !important;transform-style:flat !important;transform-box:border-box !important;will-change:unset !important}'
					);
				thisCss.push(
					'div#zq-tips-item .zq-tips-popover-tip.right{left:-10px;top:10px;border-color:transparent #fff transparent transparent}'
					);
				thisCss.push(
					'div#zq-tips-item .zq-tips-popover-tip.bottom{top:-10px;border-color:transparent transparent #fff}'
					);
				thisCss.push(
					'div#zq-tips-item .zq-tips-popover-tip{border:5px solid #fff;content:"";position:absolute}'
					);
				thisCss.push(
					'div#zq-tips-item .zq-tips-title{font:19px/normal sans-serif;margin:0 0 5px;font-weight:700;display:block;position:relative;line-height:1.5;zoom:1}'
					)
				thisCss.push(
					'div#zq-tips-item .zq-tips-description{margin-bottom:0;font:14px/normal sans-serif;line-height:1.5;color:#2d2d2d;font-weight:400;zoom:1}'
					)
				thisCss.push('div#zq-tips-item .zq-tips-footer{display:block;margin-top:10px}');
				thisCss.push(
					'div#zq-tips-item .zq-tips-footer .zq-tips-btn-group,div#zq-tips-item .zq-tips-footer .driver-close-only-btn{float:right}'
					);
				//装配css样式
			...
			reDom: function(opt) {
				setTimeout(function() {
					let $dom = opt.$element;
					$dom.css("position", opt.pst).removeClass("zq-tips-fix-stacking").css(
						"z-index", 0);
				}, time)
			},
			remove: function(opt) {
			...
			move: function(opt) {
				...
				if (highlighted_width === 0 && highlighted_height === 0) {
					$preloader.height((dom_height < window.screen.height - 22 ? dom_height + 20 :
							dom_height))
					...
					$preloader.css("transform", "translate(" + (transformX - defaultLeft) + "px," + (
							transformY - defaultTop) + "px)" +
						" scale(" + scaleX + "," + scaleY + ")");
					$preloader.css("transition", "transform " + opt.defaultTipAnim.time + " " + opt
						.defaultTipAnim.type + "");
					//滚动条跟随
					$(document.documentElement).animate({
						scrollTop: transformY + "px"
					}, 500);
				}
				...
				$dom.addClass("zq-tips-fix-stacking").css("z-index", (parseInt($preloader.css(
						"z-index")) || 9999998) + 1)
				...
				if (index === 0) {
					...
					$(".zq-tips-next").removeClass("layui-btn-disabled").off("click").on("click",
						function() {
							obj.reDom(opt);
							index++;
				...
				if (index === opts.length - 1) {
					$(".zq-tips-previous").removeClass("layui-btn-disabled").off("click").on("click",
						function() {
							obj.reDom(opt);
							index--;
							obj.remove(opts[index])
					...
					$(".zq-tips-previous").attr("disabled", false).removeClass("layui-btn-disabled")
						.off("click").on("click", function() {
							obj.reDom(opt);
							index--;
							obj.remove(opts[index])
						});
					$(".zq-tips-next").removeClass("layui-btn-disabled").show().off("click").on("click",
						function() {
							obj.reDom(opt);
							index++;
							obj.remove(opts[index])
