...
$.tips = function (tipsOption) {
    ...
    let $preloader;
    let stageElement = [];
    let options=tipsOption.tips||tipsOption;
    let obj = {
        initCss: function(){
            //用到的css样式
            let thisCss = [];
        ...
        move: function (opt) {
            ...
            let $tip = $("<div id='zq-tips-item'></div>").append(`<div class="zq-tips-popover-tip"></div>
                    ...
                    <div class="clear zq-tips-footer" style="display: block;">
                      <button class="zq-tips-close btn btn-sm btn-secondary">关闭</button>
                      <span class="zq-tips-btn-group zq-tips-navigation-btns">
                        ${options.length>1?"<button class=\"zq-tips-previous btn btn-sm btn-info\" style=\"display: inline-block;\">← 上一步</button>":""}
                        ${options.length>1?"<button class=\"zq-tips-next btn btn-sm btn-info\" style=\"display: inline-block;\">下一步 →</button>":""}
                        ${options.length>1?"<button class=\"zq-tips-ok btn btn-sm btn-success\" style=\"display: none;\">确定</button>":""}
                      </span>
                    </div>`);
            ...
            $(".zq-tips-close,.zq-tips-ok").on("click", function () {
                ...
                if($(this).hasClass("zq-tips-close")){
                    if(tipsOption.cancel&&tipsOption.cancel.cancelFunc&&$.isFunction(tipsOption.cancel.cancelFunc)){
                        tipsOption.cancel.cancelFunc();
                    }else if(tipsOption.length==1&&tipsOption[0].popover.cancelFunc){
                        tipsOption[0].popover.cancelFunc();
                    }
                }
